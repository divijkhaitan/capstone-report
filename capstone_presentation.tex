...
\begin{document}
    ...
    \begin{frame}{Our Contributions}
        \begin{itemize}
            ...
            \item 
                Measure for robustness at deployment time for a distribution shift on a single task
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Granularities to be Studied}
        We study the internals of the neural network at 3 different granularities
        \begin{itemize}
            \item 
            ...
            \item 
                Neurons: Captures second moment of the distribution over activations but lose out on path information, computationally more tractable than 
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Modelling Circuits/Neuron-Weight Interactions: Bernoulli}
        ...
        These are flattened into a single vector for which the success proportion for each coordinate $\hat{p}$ is estimated using the MLE $\hat{p}_{i, j} = \frac{1}{k}\sum {S_{i, j}^l}$, where $l$ indexes over the $k$ datapoints
        \[S = 
        \begin{bmatrix}
            S_{1,1} \\
            \vdots \\
